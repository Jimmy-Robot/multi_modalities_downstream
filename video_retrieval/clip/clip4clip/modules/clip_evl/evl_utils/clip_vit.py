...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x, return_qk=False):
        if return_qk:
            ...
            q, k, attn_output, _ = self.attn(
                x, x, x, need_weights=False, attn_mask=self.attn_mask, return_qk=True
            )
...
def vit_b32(pretrained=True, drop_path_rate=0.0):
...
def vit_b16(pretrained=True, drop_path_rate=0.0):
    model = VisionTransformer(
        input_resolution=224,
        patch_size=16,
        width=768,
...
def vit_l14(pretrained=True, drop_path_rate=0.0):
    model = VisionTransformer(
        input_resolution=224,
        patch_size=14,
        width=1024,
...
def vit_l14_336(pretrained=True, drop_path_rate=0.0):
    model = VisionTransformer(
