...
class STM(nn.Module):
    def __init__(self, n_dim, reduction=4):
        ...
        self.shift = nn.Conv2d(
            reduced_c, reduced_c, kernel_size=3, padding=1, groups=reduced_c, bias=False
        )
    ...
    def forward(self, x):
        ...
        reshape_conv_bottleneck = conv_bottleneck.view(
            (-1, T) + conv_bottleneck.size()[1:]
        )
        __, tPlusone_fea = reshape_conv_bottleneck.split(
            [1, T - 1], dim=1
        )  # N, T-1, C//r, H, W
        ...
        diff_fea_pluszero = F.pad(
            diff_fea, self.pad, mode="constant", value=0
        )  # N, T, C//r, H, W
        diff_fea_pluszero = diff_fea_pluszero.view(
            (-1,) + diff_fea_pluszero.size()[2:]
        )  # NT, C//r, H, W
...
class DSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        pre_t_fea, __ = reshape_bottleneck.split(
            [T - 1, 1], dim=1
        )  # N, T-1, C//r, H, W
        back_t_fea, __ = reshape_bottleneck.split(
            [1, T - 1], dim=1
        )  # N, T-1, C//r, H, W
        ...
        # reshape fea: N, T, C//r, H, W
        pre_reshape_conv_bottleneck = pre_conv_bottleneck.view(
            (-1, T) + pre_conv_bottleneck.size()[1:]
        )
        back_reshape_conv_bottleneck = back_conv_bottleneck.view(
            (-1, T) + back_conv_bottleneck.size()[1:]
        )
        __, tPlusone_fea = pre_reshape_conv_bottleneck.split(
            [1, T - 1], dim=1
        )  # N, T-1, C//r, H, W
        tMinusone_fea, _ = back_reshape_conv_bottleneck.split(
            [T - 1, 1], dim=1
        )  # N, T-1, C//r, H, W
        ...
        pre_diff_fea_pluszero = F.pad(
            pre_diff_fea, self.pad_pre, mode="constant", value=0
        )  # N, T, C//r, H, W
        pre_diff_fea_pluszero = pre_diff_fea_pluszero.view(
            (-1,) + pre_diff_fea_pluszero.size()[2:]
        )  # NT, C//r, H, W
        back_diff_fea_pluszero = F.pad(
            back_diff_fea, self.pad_back, mode="constant", value=0
        )  # N, T, C//r, H, W
        back_diff_fea_pluszero = back_diff_fea_pluszero.view(
            (-1,) + back_diff_fea_pluszero.size()[2:]
        )  # NT, C//r, H, W
        # recover channel
...
class TDN(nn.Module):
    def __init__(self, channel, n_segment=8, index=1, reduction=4):
        ...
        self.bn3_smallscale2 = nn.BatchNorm2d(
            num_features=self.channel // self.reduction
        )
        ...
        self.bn3_smallscale4 = nn.BatchNorm2d(
            num_features=self.channel // self.reduction
        )
    ...
    def forward(self, x):
        ...
        reshape_bottleneck = bottleneck.view(
            (-1, self.n_segment) + bottleneck.size()[1:]
        )  # n, t, c//r, h, w
        ...
        reshape_conv_bottleneck = conv_bottleneck.view(
            (-1, self.n_segment) + conv_bottleneck.size()[1:]
        )  # n, t, c//r, h, w
        ...
        y_forward_smallscale2 = F.interpolate(
            y_forward_smallscale2, diff_fea_pluszero_forward.size()[2:]
        )
        y_backward_smallscale2 = F.interpolate(
            y_backward_smallscale2, diff_fea_pluszero_backward.size()[2:]
        )

        y_forward = self.bn3(
            self.conv3(
                1.0 / 3.0 * diff_fea_pluszero_forward
                + 1.0 / 3.0 * y_forward_smallscale2
                + 1.0 / 3.0 * y_forward_smallscale4
            )
        )  # nt, c, 1, 1
        y_backward = self.bn3(
            self.conv3(
                1.0 / 3.0 * diff_fea_pluszero_backward
                + 1.0 / 3.0 * y_backward_smallscale2
                + 1.0 / 3.0 * y_backward_smallscale4
            )
        )  # nt, c, 1, 1
...
class CBlock(nn.Module):
    def __init__(
        ...
        if add_ffn:
            nn.init.constant_(self.mlp.fc2.weight, 0.0)
            nn.init.constant_(self.mlp.fc2.bias, 0.0)
...
class ResidualDecoderBlock(nn.Module):
    def __init__(
        ...
        if init_zero:
            ...
            nn.init.constant_(self.attn.in_proj_bias, 0.0)
            nn.init.trunc_normal_(self.attn.out_proj.weight, std=0.02)
            nn.init.constant_(self.attn.out_proj.bias, 0.0)
            nn.init.trunc_normal_(self.mlp.c_fc.weight, std=0.02)
            nn.init.constant_(self.mlp.c_fc.bias, 0.0)
            nn.init.trunc_normal_(self.mlp.c_proj.weight, std=0.02)
            nn.init.constant_(self.mlp.c_proj.bias, 0.0)
    ...
    def __init__(
        ...
        if before_me or after_me:
            ...
            if me_type == "stm":
                ...
                me_op = TDN
            self.me = nn.ModuleList(
                [me_op(n_dim, reduction=me_reduction) for i in range(n_layers)]
            )
        ...
        if not init_zero:
            ...
            if self.pemb_t is not None:
                nn.init.trunc_normal_(self.pemb_t, std=0.02)
        ...
        for i in range(len(clip_feats)):
            ...
            if self.tconv is not None:
                ...
                if self.t_conv_type == "1d":
                    clip_feats[i] = (
                        clip_feats[i].permute(0, 1, 3, 2).flatten(0, 1)
                    )  # L * N, C, T
                    clip_feats[i] = (
                        self.tconv[i](clip_feats[i])
                        .permute(0, 2, 1)
                        .contiguous()
                        .view(L, N, T, C)
                    )
            ...
            if i < self.uni_layer:
                ...
                tmp_feats = tmp_feats.permute(1, 3, 2, 0).reshape(N, C, T, H, W)
                tmp_feats = (
                    self.uni_dec[i](tmp_feats).view(N, C, T, L - 1).permute(3, 0, 2, 1)
                )
                clip_feats[i][1:] = clip_feats[i][1:] + tmp_feats
            clip_feats[i] = (
                clip_feats[i].permute(2, 0, 1, 3).flatten(0, 1)
            )  # T * L, N, C
...
if __name__ == "__main__":
    ...
    L, N, T, C = (
        ...
        8,
        768,
    )  # num_image_tokens, video_batch_size, t_size, feature_dim
    ...
    for i in range(8):  # vit-b has 12 blocks
        ...
        input_features.append(tuple(torch.zeros([L, N, T, C]) for _ in range(3)))
