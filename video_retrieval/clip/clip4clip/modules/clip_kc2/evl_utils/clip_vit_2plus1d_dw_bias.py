...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Conv3d(
            d_model, d_model, kernel_size=3, stride=1, padding=1, groups=d_model
        )
        # temporal
        ...
        idx_tensor = torch.zeros(
            [spatial_size**2, spatial_size**2], dtype=torch.long
        )
        ...
        if init_zero:
            ...
            nn.init.constant_(self.ln_t.weight, 1.0)
            nn.init.constant_(self.ln_t.bias, 0.0)
        else:
            nn.init.trunc_normal_(self.rpb_t, std=0.02)
            nn.init.trunc_normal_(self.rpb, std=0.02)

            nn.init.trunc_normal_(self.pos_embed.weight, std=0.02)
            nn.init.constant_(self.pos_embed.bias, 0)
            nn.init.trunc_normal_(self.attn_t.in_proj_weight, std=0.02)
            nn.init.constant_(self.attn_t.in_proj_bias, 0)
            nn.init.trunc_normal_(self.attn_t.out_proj.weight, std=0.02)
            nn.init.constant_(self.attn_t.out_proj.bias, 0)
            nn.init.constant_(self.ln_t.weight, 1.0)
            nn.init.constant_(self.ln_t.bias, 0.0)
    ...
    def attention(self, x, rpb=None):
        ...
        return self.attn(
            x, x, x, need_weights=False, attn_mask=self.attn_mask, rpb=rpb
        )[0]
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, return_num=4, masked_indices=None, mode="video"):
        ...
        features = self.transformer(x, return_num=return_num, T=T, mode=mode)

        return features


def vit_2plus1d_dw_bias_b32(
    pretrained=True, num_frames=8, drop_path_rate=0.0, t_size=8, init_zero=True
):
...
def vit_2plus1d_dw_bias_l14_336(
    pretrained=True, num_frames=8, drop_path_rate=0.0, t_size=8, init_zero=True
):
    model = VisionTransformer(
