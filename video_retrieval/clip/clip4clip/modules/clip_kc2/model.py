...
class CLIP(nn.Module):
    def __init__(
        ...
        self.visual = evl_utils.__dict__[backbone](
            pretrained=False, init_zero=init_zero, num_frames=t_size, t_size=t_size
        )
        self.evl = TransformerDecoder_uniformer_diff_conv_balance(
            ...
            uni_type=uni_type,
            add_ffn=add_ffn,
            t_conv_type=t_conv_type,
            pre_prompt=pre_prompt,
            balance=balance,
            after_me=after_me,
            before_me=before_me,
            me_type=me_type,
            me_reduction=me_reduction,
            init_zero=init_zero,
        )
    ...
    def encode_video(
        ...
        features = self.visual(
            video, return_num=self.return_num, masked_indices=masked_indices, mode=mode
        )
    ...
    if mergeclip:
        ...
        for k in new_sd:
            ...
            if any(x in k for x in clip_sd.keys()):
                print("merging: ", k)
                new_sd[k] = clip_sd[k] * mergeweight + state_dict[k] * (
                    1.0 - mergeweight
                )
