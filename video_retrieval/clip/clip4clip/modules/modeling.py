...
from . import clip_evl
from .clip_evl.model_no_freeze_only_global import (
    vit_only_global_l_sparse8_k400,
    vit_only_global_b_sparse8_k400,
)
...
class dual_softmax_loss(nn.Module):
    ...
    def forward(self, sim_matrix, temp=1000):
        sim_matrix = (
            sim_matrix * F.softmax(sim_matrix / temp, dim=0) * len(sim_matrix)
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        self.interaction = (
            task_config.interaction if hasattr(task_config, "interaction") else "no"
        )
        ...
        if hasattr(task_config, "clip_evl") and task_config.clip_evl == True:
            self.clip, _ = clip_evl.load(
                task_config.pretrained_path,
                t_size=task_config.max_frames,
                mergeclip=task_config.mergeclip,
                mergeweight=task_config.mergeweight,
    ...
    def forward(
        ...
        if self.training:
            ...
            if self.wti_interaction != "no" and self.cdcr:
                sim_matrix, _, cdcr_loss = self.get_similarity_logits(
                    sequence_output,
                    visual_output,
                    attention_mask,
                    video_mask,
        ...
        if self.interaction == "wti" or self.interaction == "ti":
            if self.clip_evl:
                sequence_hidden = self.clip.encode_text(
                    input_ids, return_all_feats=True
                )[1].float()
            else:
                sequence_hidden = self.clip.encode_text(input_ids, return_hidden=True)[
                    1
                ].float()
        else:
            sequence_hidden = self.clip.encode_text(input_ids).float()
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        if self.clip_evl:
            if len(video.size()) == 4:
                # [b, t, c, h, w]
                video = video.view(
                    bs_pair, -1, video.size(-3), video.size(-2), video.size(-1)
                )
                video = video.permute(0, 2, 1, 3, 4).contiguous()
            # [N, 1, d], [L, N, T, d]
            evl_output, visual_output = self.clip.encode_video(
                video, return_all_feats=True
            )
            ...
            if self.interaction == "wti":
                ...
                visual_hidden = (
                    self.clip.visual_ln_post(visual_output[0]) @ self.clip.visual_proj
                )
        else:
    ...
    def _cross_similarity(
        ...
        for i in range(len(split_size)):
            ...
            retrieve_logits_row = (
                self.similarity_dense(pooled_output)
                .squeeze(-1)
                .view(step_truth, b_visual)
            )
    ...
    def wti_interaction(self, text_feat, video_feat, text_mask, video_mask):
        ...
        if self.training:
            ...
            if self.cdcr == 1:
                # simple random
                _text_feat = text_feat[
                    torch.arange(text_feat.shape[0]),
                    torch.randint_like(text_sum, 0, 10000) % text_sum,
                    :,
                ]
                _video_feat = video_feat[
                    torch.arange(video_feat.shape[0]),
                    torch.randint_like(video_sum, 0, 10000) % video_sum,
                    :,
                ]
                z_a_norm = (_text_feat - _text_feat.mean(0)) / _text_feat.std(
                    0
                )  # NxN_sxD
                z_b_norm = (_video_feat - _video_feat.mean(0)) / _video_feat.std(
                    0
                )  # NxN_txD
                ...
                # selecet max
                max_idx1 = max_idx1[
                    torch.arange(max_idx1.shape[0]), torch.arange(max_idx1.shape[1])
                ]
                max_idx2 = max_idx2[
                    torch.arange(max_idx2.shape[0]), torch.arange(max_idx2.shape[1])
                ]

                max_t_feat = text_feat[
                    torch.arange(max_idx2.shape[0]).repeat_interleave(
                        max_idx2.shape[1]
                    ),
                    max_idx2.flatten(),
                ]
                max_v_feat = video_feat[
                    torch.arange(max_idx1.shape[0]).repeat_interleave(
                        max_idx1.shape[1]
                    ),
                    max_idx1.flatten(),
                ]
